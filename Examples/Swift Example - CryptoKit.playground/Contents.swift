...
if HMAC<SHA512>.isValidAuthenticationCode(authenticationCodeData, authenticating: data, using: key256) {
...
let albusSingningPublicKeyData = albusSigningPrivateKey.publicKey.rawRepresentation
...
let publicKey = try! Curve25519.Signing.PublicKey(rawRepresentation: albusSingningPublicKeyData)
...
let ADsymmetricKey = ADsharedSecret.hkdfDerivedSymmetricKey(using: SHA256.self, salt: protocolSalt, sharedInfo: Data(), outputByteCount: 32)
...
let HPsymetricKey = ADsharedSecret.hkdfDerivedSymmetricKey(using: SHA256.self, salt: protocolSalt, sharedInfo: Data(), outputByteCount: 32)

// As if by magic, they produce the same symmetric key!

if ADsymmetricKey == HPsymetricKey {
    print("Dumbledore and Harry have the same symetric key.")
}
